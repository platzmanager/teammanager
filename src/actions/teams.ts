...
export async function getBlockedCount(team: Team): Promise<number> {
	return withClubContext(async (supabase) => {
		...
		if (error) throw error;
		return (data ?? []).reduce(
			(sum: number, t: { team_size: number }) => sum + t.team_size,
			0,
		);
	});
}
...
export async function getTeamCaptains(
	...
	return withClubContext(async (supabase) => {
		const { data, error } = await supabase
			...
			.select("member_id, members!inner(user_id, first_name, last_name)")
			.eq("team_id", teamId)
			.eq("role", "captain");
		if (error) throw error;
...
export async function addCaptain(teamId: string, userId: string) {
	...
	return withClubContext(async (supabase, clubId) => {
		...
		const { error } = await supabase
			.from("member_team_assignments")
			.upsert(
				{ member_id: member.id, team_id: teamId, role: "captain" },
...
export async function inviteCaptain(
	...
	return withClubContext(async (supabase, clubId) => {
		...
		// Fetch member name for the returned captain
...
export async function getRegisteredPlayers(gender: Gender, ageClass: AgeClass) {
	return withClubContext(async (supabase, clubId) => {
		const { data, error } = await supabase
			...
			.select(
				"player_uuid, players!inner(uuid, first_name, last_name, skill_level, birth_date, sort_position, license, deleted_at)",
			)
			.eq("gender", gender)
			.eq("age_class", ageClass)
		...
		const players = (data ?? []).map(
			(r: any) => r.players as import("@/lib/types").Player,
		);
...
export async function getAllRegisteredPlayers(): Promise<
	RegisteredPlayerWithAgeClasses[]
> {
	await requireAdmin();
	...
	return withClubContext(async (supabase, clubId) => {
		const { data, error } = await supabase
			.from("player_registrations")
			.select(
				"gender, age_class, players!inner(uuid, first_name, last_name, birth_date, gender, license)",
			)
			.eq("players.club_id", clubId)
			.is("players.deleted_at", null);
...
export async function getNextMatches(): Promise<
	Record<string, import("@/lib/types").Match>
> {
	return withClubContext(async (supabase, clubId) => {
		const today = new Date().toISOString().split("T")[0];
		const { data, error } = await supabase
