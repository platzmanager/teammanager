...
import type { AgeClass, Gender, Player } from "@/lib/types";
...
function parsePdfPlayers(textItems: TextItem[]): PdfPlayer[] {
	...
	// Sort by rank number
	players.sort(
		(a, b) => (parseInt(a.rank, 10) || 0) - (parseInt(b.rank, 10) || 0),
	);
	return players;
}
...
function describeMatch(type: MatchType): string {
	switch (type) {
		case "id":
			return "ID-Nr. stimmt überein";
		case "id_and_name":
			return "ID-Nr. und Name stimmen überein";
		case "name_exact":
			return "Name stimmt überein (Fallback, keine ID-Nr. im System)";
		case "name_fuzzy":
			return "Name ähnlich (Fuzzy-Fallback, keine ID-Nr. im System)";
	}
}
...
function findMatchInPdf(
	...
	if (systemPlayer.license) {
		const idMatch = pdfPlayers.find(
			(pp) => pp.licenseId === systemPlayer.license,
		);
		if (idMatch) {
...
export function EntryListCheck() {
	...
	const [detectedAgeClass, setDetectedAgeClass] = useState<AgeClass | null>(
		null,
	);
	const [selectedGender, setSelectedGender] = useState<Gender | null>(null);
	const [selectedAgeClass, setSelectedAgeClass] = useState<AgeClass | null>(
		null,
	);
	...
	const processFile = useCallback(async (file: File) => {
		...
		try {
			...
			alert(
				`PDF konnte nicht gelesen werden: ${err instanceof Error ? err.message : String(err)}`,
			);
			setFileName(null);
		} finally {
			setLoading(false);
	...
	async function handleCheck() {
		if (!effectiveGender || !effectiveAgeClass || pdfPlayers.length === 0)
			return;
	...
	const canCheck =
		pdfPlayers.length > 0 && effectiveGender && effectiveAgeClass && !loading;
          ...
          `}
				>
					...
					<div className="text-center">
						<p className="text-sm font-medium">
							Meldelisten-PDF hierher ziehen
						</p>
					...
					{detectedGender && detectedAgeClass && (
						<p className="text-sm text-muted-foreground">
							...
							<span className="font-medium">
								{GENDER_LABELS[detectedGender]}{" "}
								{AGE_CLASS_CONFIG[detectedAgeClass].label}
							</span>
						</p>
					)}
			...
			{results && (
				<div className="space-y-3">
						...
						<details className="rounded-lg border">
							...
							<div className="border-t px-4 py-2">
								<p className="text-xs text-muted-foreground pb-2">
									Diese Spieler sind als zu melden markiert und wurden auch in
									der Verbands-Meldeliste gefunden.
								</p>
					...
					{results.notInPdf.length > 0 && (
						<details open className="rounded-lg border">
							<summary className="flex cursor-pointer items-center gap-2 px-4 py-2.5 text-sm font-medium hover:bg-muted/50">
								...
								{results.notInPdf.length} noch nicht beim Verband gemeldet (im PDF nicht gefunden)
							</summary>
							<div className="border-t px-4 py-2">
								<p className="text-xs text-muted-foreground pb-2">
									Diese Spieler sollten für diese Altersklasse gemeldet werden,
									tauchen aber in der Verbands-Meldeliste (PDF) nicht auf.
								</p>
					...
					{/* Not in system for this age class (expanded) */}
					{results.notInSystem.length > 0 && (
						<details open className="rounded-lg border">
							<summary className="flex cursor-pointer items-center gap-2 px-4 py-2.5 text-sm font-medium hover:bg-muted/50">
								<AlertTriangle className="h-4 w-4 text-amber-500" />
								{results.notInSystem.length} nicht zu melden für diese AK
							</summary>
							<div className="border-t px-4 py-2">
								<p className="text-xs text-muted-foreground pb-2">
									Diese Spieler sollten für diese Altersklasse nicht gemeldet werden, tauchen aber in der Verbands-Meldeliste (PDF) auf.
								</p>
