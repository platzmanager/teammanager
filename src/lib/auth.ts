...
export const getUserProfile = cache(async (): Promise<UserProfile | null> => {
	...
	// Run profile + club-scoped queries in parallel
	const [{ data: profileData }, roleAndTeams] = await Promise.all([
		supabase
			.from("user_profiles")
	...
	if (roleAndTeams) {
		const [{ data: ucData }, { data: memberData }] = roleAndTeams;
		role = (ucData?.role as UserRole) ?? "user";
		const assignments =
			(memberData?.teams as unknown as { role: string; team: Team }[] | null) ??
			[];
		teams = assignments.map((a) => a.team);
		captainTeamIds = assignments
			.filter((a) => a.role === "captain")
			.map((a) => a.team.id);
	}
...
export function canAccessTeamScope(
	profile: UserProfile,
	gender: Gender,
	ageClass: AgeClass,
): boolean {
...
export function getUserAgeClasses(
	profile: UserProfile,
	gender: Gender,
): AgeClass[] {
	if (profile.role === "admin")
		return ["all", "30", "40", "50", "60", "u9", "u10", "u12", "u15", "u18"];
...
export function getUserTeamScopes(
	profile: UserProfile,
